...
const Terminal: React.FC<TerminalProps> = ({
  ...
  const isCompactDesktop = isDesktop && terminalWidth < 380;
  const terminalFont = {
    ...
    >
      ...
      {!isDesktop && !isTerminalOpen && (
        <div
          ...
          onClick={() => setIsTerminalOpen(true)}
        >
          <div className="flex items-center gap-1.5 text-slate-500 text-[9px] font-bold uppercase tracking-wider" style={terminalFont}>
        ...
        <>
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              {!isCompactDesktop && (
                <span className="text-[10px] text-slate-500 tracking-wide truncate" style={terminalFont}>
                  skill_console
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="text-slate-600">{isDesktop ? <ChevronRight size={12} /> : <ChevronDown size={12} />}</div>
            </div>
          </div>

          <div
            className={`flex-1 overflow-y-auto ${isCompactDesktop ? 'px-2 py-2 text-[11px]' : 'px-4 py-3 text-[12px]'} leading-[1.7]`}
            style={terminalFont}
          >
            {logs.map((log, index) => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {log.type === 'query' && (
                    <div className="flex items-start gap-1">
                      ...
                      <span className="text-sky-300 font-medium break-words">{log.text}</span>
                    </div>
                  )}
                  {log.type === 'result' && (
                    <span className="text-emerald-300/90 whitespace-pre-wrap break-words block ml-3 leading-snug">{log.text}</span>
                  )}
                  {log.type === 'ai-loading' && (
                    <span className="text-purple-400 italic flex items-center gap-1.5">
                      <Loader2 size={11} className="animate-spin" /> {log.text}
                    </span>
                  )}
                  {log.type === 'ai-explain' && (
                  ...
                  {log.type === 'error' && (
                    <span className="text-red-400/90 bg-red-500/10 p-2 rounded-lg block border border-red-500/20 text-[11px]">
                      {log.text}
                    </span>
                  )}
                </div>
          ...
          <div className={`border-t border-[#2d3348] ${isCompactDesktop ? 'px-2 py-1.5' : 'px-4 py-2'} flex items-center gap-2 bg-[#1e2130] shrink-0`}>
            <span className="text-emerald-400 text-[12px] select-none" style={terminalFont}>
              {'>'}
            </span>
            <input
              type="text"
              ...
              className={`flex-1 bg-transparent text-slate-200 ${isCompactDesktop ? 'text-[11px]' : 'text-[12px]'} outline-none placeholder-[#4a5173] min-w-0 caret-emerald-400`}
              style={terminalFont}
              spellCheck="false"
              autoComplete="off"
            />
